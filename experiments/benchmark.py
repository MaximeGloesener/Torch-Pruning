...
def measure_latency_cpu(model, dummy_input, n_warmup=50, n_test=200):
    ...
    print(f'{mean_syn} ms +/- {std_syn} ms')
...
def benchmark(model, dummy_input, n_warmup=50, n_test=200, plot=False):
    ...
    mean_syn, std_syn = measure_latency_cpu(model, dummy_input, n_warmup, n_test)
    print('GPU inference time:')
    dummy_input = dummy_input.to('cuda')
    mean_syn, std_syn = measure_latency_gpu(model, dummy_input, n_warmup, n_test)
    print('Sparsité:', get_model_sparsity(model))
    get_layer_sparsity(model)
    print(f'Nombre de paramètres: {get_num_parameters(model)/1e6} M')
    print(f'Taille du modèle: {get_model_size(model)/MiB} MiB')
    print(f'Nombre de MACs: {get_model_macs(model, dummy_input)/1e6} M')
    if plot:
